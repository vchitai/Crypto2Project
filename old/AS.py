...
def register_new_device():
    ...
    if Certificate.query.filter(or_(Certificate.device_name == device_name, Certificate.public_key == public_key)).first() is not None:
        abort(400) # existing cert, pkey
    cert = Certificate(device_name = device_name, public_key = public_key)
    m = json.dumps({ 'device_id': cert.get_id() })
    h = SHA256.new()
    h.update(m)
    pkey = cert.export_key()
    c = pkey.encrypt(m, 32)[0]
    print c.decode('utf-16')
    responseC = json.dumps({ 'c': c, 'h': h.hexdigest() })
    print responseC
##    db.session.add(cert)
##    db.session.commit()
    return responseC, 201
